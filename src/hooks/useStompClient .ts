...
interface IStompClientParams {
    peerConnectionRef: any,
    setError: any,
}

export const useStompClient: any = ({ peerConnectionRef, setError }: IStompClientParams) => {
            ...
            onConnect: () => {
                ...
                client.subscribe(`/user/queue/call`, (message: IMessage) => {
                    try {
                        ...
                        handleSignalingData(data, peerConnectionRef, setError, stompClientRef);
                    } catch (err) {
    ...
    return [ownClientId, connectionStatus, clientIds, stompClientRef];
}
